...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("encode src url: %s\n", rawSrcURL)
	fmt.Printf("output file name is %v\n", output)
	cli := []string{"ffmpeg", "-i", rawSrcURL, "-acodec", "aac", "-strict", "experimental", "data/" + output}
	fmt.Printf("command line:%v", cli)
	cmd := exec.Command("ffmpeg", "-i", rawSrcURL, "-acodec", "aac", "-strict", "experimental", "data/"+output)
	//cmd.Stdin = strings.NewReader("some input")
	var out bytes.Buffer
	cmd.Stdout = &out
	err = cmd.Run()
	if err != nil {
		log.Printf("exec encode cmd fail:%v", err)
		return
	}
